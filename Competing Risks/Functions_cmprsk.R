...
DataApplication_cmprsk <- function(data, init.value.theta_1, init.value.theta_2) {
  ...
  # Estimate V 
  gammaest <- nloptr(x0=rep(0,parlgamma),eval_f=LikGamma_app,Y=Z,M=XandW,lb=c(rep(-Inf,parlgamma)),ub=c(rep(Inf,parlgamma)),
                     eval_g_ineq=NULL,opts = list(algorithm = "NLOPT_LN_BOBYQA","ftol_abs"=1.0e-30,"maxeval"=100000,"xtol_abs"=rep(1.0e-30)))$solution
  ...
  secder=(t(-dlogis(Xandintercept%*%gammaest))*W)%*%prodvec
  
  WM = secder[1:parlgamma]
  for (i in 2:parlgamma) {
    newrow<-secder[c(i,(i+1):(i-1+parlgamma))]
    WM<-rbind(WM,newrow) 
  }
  ...
  XandW.11<-c(1,-0.1360154) #XandW matrix
  Z.11<-1 # Value for Z
  W.11 <- 1
  V.est.11 <- (W.11-Z.11)*((1+exp(XandW.11%*%gammaest))*log(1+exp(XandW.11%*%gammaest))-(XandW.11%*%gammaest)*exp(XandW.11%*%gammaest))-Z.11*((1+exp(-(XandW.11%*%gammaest)))*log(1+exp(-(XandW.11%*%gammaest)))+(XandW.11%*%gammaest)*exp(-(XandW.11%*%gammaest))) #Estimated control function
  ...
  XandW.01<-c(1,-0.1360154)
  Z.01<-0
  W.01<-1
  V.est.01 <- (W.01-Z.01)*((1+exp(XandW.01%*%gammaest))*log(1+exp(XandW.01%*%gammaest))-(XandW.01%*%gammaest)*exp(XandW.01%*%gammaest))-Z.01*((1+exp(-(XandW.01%*%gammaest)))*log(1+exp(-(XandW.01%*%gammaest)))+(XandW.01%*%gammaest)*exp(-(XandW.01%*%gammaest)))
  ...
  XandW.00<-c(1,-0.1360154)
  Z.00<-0
  W.00 <- 0
  V.est.00 <- (W.00-Z.00)*((1+exp(XandW.00%*%gammaest))*log(1+exp(XandW.00%*%gammaest))-(XandW.00%*%gammaest)*exp(XandW.00%*%gammaest))-Z.00*((1+exp(-(XandW.00%*%gammaest)))*log(1+exp(-(XandW.00%*%gammaest)))+(XandW.00%*%gammaest)*exp(-(XandW.00%*%gammaest)))
