...
# After downloading the data, unzip the relevant Stata data files (.dta) in the
# "Data" directory. The relevant files are:
# - expfup1/expfup1_stata
# - expfup2/expfub2_stata
# - expbif/expbif_stata

main_dir <- "jtpa_national_evaluation"
data_dir <- "Data"

path1 <- paste0(main_dir, "/", data_dir, "/expfup1")
path2 <- paste0(main_dir, "/", data_dir, "/expfup2")
path3 <- paste0(main_dir, "/", data_dir, "/expbif")

jtpapartfu1 <- read_dta(paste0(path1, "/", "expfup1_stata/main.dta"))
jtpapartfu2 <- read_dta(paste0(path2,"/",  "expfup2_stata/main.dta"))
covariates <- read_dta(paste0(path3, "/", "expbif_stata/expbif.dta"))
jobf1 <- read_dta(paste0(path1, "/", "expfup1_stata/bjob.dta"))
jobf2 <- read_dta(paste0(path2, "/", "expfup2_stata/bjob.dta"))
headf1 <- read_dta(paste0(path1, "/", "expfup1_stata/head_fp1.dta"))
headf2 <- read_dta(paste0(path2, "/", "expfup2_stata/head_fp2.dta"))

jtpa1 <- jtpapartfu1
jtpa2 <- jtpapartfu2

#### JTPA participation and covariates ####

# From the main data set, only keep record ID and JTPA participation ("f1d7" and
# "f2d7"), coded as: 1 = participation / 2 = no participation / 7 = refused /
#                    8 don't know / 9 = missing.
jtpa1 <- jtpa1[,c("recid","f1d7")]
jtpa2 <- jtpa2[,c("recid","f2d7")]

# Only keep the respondents who answered 1 or 2
jtpa1 <- jtpa1[jtpa1$f1d7 %in% c(1,2),] 
jtpa2 <- jtpa2[jtpa2$f2d7 %in% c(1,2),]
...
#### calculate Y = min(T, C, A), Delta = 1(Y == T), Xi = 1(Y == C)####

# beginning of study and first follow up date
beginendfu1 <- cbind.data.frame(as.numeric(headf1$recid), headf1$f1beg, headf1$f1ref)
colnames(beginendfu1) <- c("recid", "beg", "end1")

# date of first job (first follow up interview)
datejobfu1 <- cbind.data.frame(jobf1$recid, jobf1$f1b2bdd, jobf1$f1b2bmm, jobf1$f1b2byy)

# date of second follow up interview
endfu2 <- cbind.data.frame(as.numeric(headf2$recid), headf2$fdat_fin) 
colnames(endfu2) <- c("recid","end2")

# date of first job (second follow up interview)
datejobfu2<-cbind.data.frame(jobf2$recid, jobf2$f2b2bdd, jobf2$f2b2bmm, jobf2$f2b2byy)
...
# Initialize censoring indicators
data$delta <- 0
data$xi <- 0

# All event times that are already observed should be indicated with delta = 1
data$delta[which(!is.na(data$date))] <- 1
...
# only the subjects in our data that have NA as date, meaning they did not have
# a job when the first follow up interview was taken
...
# Loop over all observations in the data set
for(i in 1:nrow(data)){
  id <- data$recid[i]
...
# Gives the censoring percentage
sum(is.na(data$date))/nrow(data)

# Check that it matches
1 - sum(data$delta)/nrow(data)
...
data <- na.omit(data)
data <- subset(data, select = -c(10,13))
...
write.csv(data,paste("clean_dataset_JTPA_ad.csv",sep = ""), row.names = F)
